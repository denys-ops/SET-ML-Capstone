...
md("### Підсвічені приклади: найвпевненіший фішинг, легітимні листи й реалістична приманка")
...
md("""Зелене підсвічує слова, що штовхають бал до фішингу, червоне — до легітимного. Типові маркери
...
md("""### Реальний zero-shot прогін (Claude Opus 4.8)
...
code('''real = pd.DataFrame(json.load(open("data/llm_eval/metrics.json")))[["model", "F1", "Acc"]]
real''')

md("### Вартість LLM (оцінка з цін токенів — для контексту)")
...
md("""**Висновок.** За якістю моя модель нарівні з LLM (реальний прогін Claude — F1 1.00, а числа GPT з
...
plt.tight_layout(); plt.show()''')
md("""**Як читати графік.** Кожна точка — лист. Сині (легітимні) унизу зліва: людина каже «0», модель дає
